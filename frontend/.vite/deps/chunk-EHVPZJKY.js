  ...
  _init() {
    ...
    for (let i = 0, j = 0, n = triangles.length, x, y; i < n; i += 3, j += 2) {
      ...
      if (Math.abs(ab4) < 1e-9) {
        ...
        a *= Math.sign((points[r] - x13) * ey - (points[r + 1] - y13) * ex);
        x = (x13 + x3) / 2 - a * ey;
        y = (y13 + y3) / 2 + a * ex;
      } else {
        const d = 1 / ab4;
...
function collinear(d) {
  ...
  for (let i = 0; i < triangles.length; i += 3) {
    const a = 2 * triangles[i], b = 2 * triangles[i + 1], c3 = 2 * triangles[i + 2], cross = (coords[c3] - coords[a]) * (coords[b + 1] - coords[a + 1]) - (coords[b] - coords[a]) * (coords[c3 + 1] - coords[a + 1]);
  ...
  return {
    ...
    point: function(lambda12, phi12) {
      ...
      if (abs(delta - pi) < epsilon3) {
        ...
        if (abs(lambda04 - sign0) < epsilon3) lambda04 -= sign0 * epsilon3;
...
function clipAntimeridianIntersect(lambda04, phi02, lambda12, phi12) {
  ...
  return abs(sinLambda0Lambda1) > epsilon3 ? atan((sin(phi02) * (cosPhi1 = cos(phi12)) * sin(lambda12) - sin(phi12) * (cosPhi03 = cos(phi02)) * sin(lambda04)) / (cosPhi03 * cosPhi1 * sinLambda0Lambda1)) : (phi02 + phi12) / 2;
}
...
function circle_default2(radius) {
  var cr = cos(radius), delta = 6 * radians, smallRadius = cr > 0, notHemisphere = abs(cr) > epsilon3;
  ...
  function intersect(a, b, two) {
    ...
    if (meridian ? polar ? phi02 + phi12 > 0 ^ q[1] < (abs(q[0] - lambda04) < epsilon3 ? phi02 : phi12) : phi02 <= q[1] && q[1] <= phi12 : delta2 > pi ^ (lambda04 <= q[0] && q[0] <= lambda12)) {
...
function graticuleX(y06, y12, dy) {
  var y = range(y06, y12 - epsilon3, dy).concat(y12);
  return function(x) {
...
function graticuleY(x06, x12, dx) {
  var x = range(x06, x12 - epsilon3, dx).concat(x12);
  return function(y) {
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a = a0 + a1, b = b0 + b1, c3 = c0 + c1, m = sqrt(a * a + b * b + c3 * c3), phi2 = asin(c3 /= m), lambda22 = abs(abs(c3) - 1) < epsilon3 || abs(lambda04 - lambda12) < epsilon3 ? (lambda04 + lambda12) / 2 : atan2(b, a), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
...
//# sourceMappingURL=chunk-EHVPZJKY.js.map
